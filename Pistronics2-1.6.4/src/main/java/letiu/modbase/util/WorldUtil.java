...
public class WorldUtil {
	...
	public static int setBlockFacing(World world, int x, int y, int z, EntityLivingBase player) {
        ...
        world.setBlockMetadataWithNotify(x, y, z, meta = facing | bit4, 2);
        return facing;
	}
...
//	}
	...
	public static void setBlockBounds(IBlockAccess world, int x, int y, int z, float minX, float minY, float minZ, float maxX, float maxY, float maxZ) {
		getBlock(world, x, y, z).setBlockBounds(minX, minY, minZ, maxX, maxY, maxZ);
	}
	
	public static void setBlockBounds(IBlockAccess world, int x, int y, int z, AxisAlignedBB box) {
		getBlock(world, x, y, z).setBlockBounds((float) box.minX, (float) box.minY, (float) box.minZ, (float) box.maxX, (float) box.maxY, (float) box.maxZ);
	}
	...
	public static void cleanBlock(World world, Vector3 vec) {
		if (!world.isRemote) {
			world.isRemote = true;
			world.setBlockToAir(vec.x, vec.y, vec.z);
			world.isRemote = false;
		}
		else world.setBlockToAir(vec.x, vec.y, vec.z);
	}
	...
	public static void setBlock(World world, int x, int y, int z, Block block, int meta, int flag) {
		if (block != null) {
			world.setBlock(x, y, z, block.blockID, meta, flag);
		}
	}
	
	public static void setBlock(TileEntity tile, Block block) {
		tile.getWorldObj().setBlock(tile.xCoord, tile.yCoord, tile.zCoord, block.blockID);
	}
	...
	public static Block getBlock(IBlockAccess world, int x, int y, int z) {
		return Block.blocksList[getBlockID(world, x, y, z)];
	}
